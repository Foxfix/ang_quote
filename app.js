(function() {
	...
	var RandomQuoteComponent = Component({
		selector: 'randome-quote',
		template: '<p><em>{{ quote.line}}</em> - {{quote.author}}</p>'
	})
	.Class({
		constructor: function() {
			var ransomIndex = Math.floor(Math.random() * quotes.length);
			this.quote = quotes[ransomIndex];
		}
	})
	...
	var AppComponent = Component({
		selector: "my-app",
		template: '<h1>Randome Quote</h1>' +
		'<randome-quote></randome-quote>'
	})
	.Class({
		constructor: function() { }
	});
